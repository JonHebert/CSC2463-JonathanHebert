...
// Initialize Tone.js player for background music
var bgSynth;
...
function mousePressed() {
    if (mouseX >= 0 && mouseX < palette_cell_size && mouseY >= 0 && mouseY < (colors.length * palette_cell_size)) {
        ...
        stroke(colors[selectedColorIndex]);
        drawing = false;
        colorChangeSynth.triggerAttackRelease('C4', '8n');
    } else {
        x = mouseX;
        y = mouseY;
        drawing = true;
...
function keyPressed() {
    // Check if key pressed is 'c' (for clearing screen)
    if (key === 'c' || key === 'C') {
        // Clear screen
        clear();
        // Trigger clear screen sound effect
